...
def generate_rules():
    ...
    rules = []
    for combo in itertools.product(sleep_options, academic_options, hubungan_options, support_options, hidup_options):
...
def determine_consequent(rule):
    ...
    if rule['Sleep Quality'] == 'Buruk':
        negative_count += 1
    if rule['Academic Performance'] in ['Rendah', 'Sedang']:
        negative_count += 1
    ...
    for combo in itertools.product(*category_lists):
        ...
        if alpha > 0:
            ...
            if consequent == 'Tinggi':
                z_i = 75 + 25 * alpha  # 75–100
            elif consequent == 'Sedang':
                z_i = 50  # atau 50 + 10 * alpha
            else:  # Normal
                z_i = 25 - 25 * alpha  # 0–25
